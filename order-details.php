...
function stripTags( $string, $remove_breaks = false ) {
    ...
    $string = strip_tags( str_replace('-', ';%0a ', $string) );
	...
	function ajusta_wc_display_item_meta ($item){
			...
			foreach ( $order_items as $item_id => $item ) {
				...
				$Produtos .= "*".$item->get_quantity()."x* ".$item->get_name()."%0a%0a ". stripTags(wc_display_item_meta($item, 'echo')). "%0a";
					$soma += $item->get_total();
			}
			...
			//$telefone =  $numeroZap;
			//$telefone = WHATSAPP;
            //$telefone = '5581997441540';
			//$dados .= "Telefone -> " . $telefone;
            $telefone = selectNumWhatsApp();
